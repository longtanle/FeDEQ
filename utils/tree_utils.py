...
def tree_random_normal_like(rng_key, target):
    ...
    return tree_map(
        lambda l, k: jax.random.uniform(k, (1,), minval = 0.01, maxval = 0.01),
        target,
        keys_tree,
    )

def tree_random_bernoulli_like(rng_key, target):
    keys_tree = random_split_like_tree(rng_key, target)
    return tree_map(
        lambda l, k: 2.0 * jnp.round(jax.random.normal(k, (1,))) - 1,
        target,
        keys_tree,
    )
