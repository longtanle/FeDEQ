...
class UTDEQBlock(hk.Module):
    ...
    def __call__(self,
        ...
        for i in range(self._num_layers):
            ...
            h_norm = layer_norm(h, name= name + '_ln_1')
            h_attn = CausalSelfAttention(self._num_heads,
                                         init_scale,
                                         name=name + '_attn')(h_norm, mask)
            h_attn = hk.dropout(hk.next_rng_key(), dropout_rate, h_attn)
            h = h + h_attn
            
            h_norm = layer_norm(h, name=name + '_ln_2')
            h_dense = DenseBlock(init_scale, name=name + '_mlp')(h_norm)
            h_dense = hk.dropout(hk.next_rng_key(), dropout_rate, h_dense)
            h = h + h_dense
        h = layer_norm(h, name=name + '_ln_f')
