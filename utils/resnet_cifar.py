...
class BlockV1(hk.Module):
    ...
    def __init__(
        ...
        gn_0 = hk.GroupNorm(name="groupnorm_0", groups = 8)
        conv_1 = hk.Conv2D(
            output_channels=channels,
            kernel_shape=3,
            stride=1,
        ...
        gn_1 = hk.GroupNorm(name="groupnorm_1", groups = 8)
        self.layers = ((conv_0, bn_0), (conv_1, bn_1))
        self.layers_2 = ((conv_0, gn_0), (conv_1, gn_1))
        if stride != 1:
...
class ResNet(hk.Module):
    ...
    def __init__(
        ...
        channels_per_group: Sequence[int] = (64, 128, 256),
        ...
        logits_config.setdefault("name", "player_logits")

        # Number of blocks in each group for ResNet.
        # check_length(4, blocks_per_group, "blocks_per_group")
        # check_length(4, channels_per_group, "channels_per_group")
        # Add some linear layers
        linear_config = dict(linear_config or {})
        linear_config.setdefault("with_bias", True)
        linear_config.setdefault("name", "linear")
        self.linear_1 = hk.Linear(256, **linear_config)
        self.linear_2 = hk.Linear(128, **linear_config)
        self.linear_batchnorm = hk.BatchNorm(name="linear_batchnorm", **bn_config)
    ...
    def __call__(self, inputs, is_training, test_local_stats=False):
        ...
        out = jax.nn.relu(out)
        #embedding = self.linear_batchnorm(out, is_training, test_local_stats)
        out = self.linear_2(out)
        embedding = jax.nn.relu(out)
        out = self.linear_batchnorm(embedding, is_training, test_local_stats)
        #out = hk.dropout(hk.next_rng_key(), 0.3, out)
        return self.logits(out), embedding
