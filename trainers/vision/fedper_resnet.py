...
class FedPer(BaseTrainerGlobal):
  def __init__(self, args, data):
    super().__init__(args, data)
    print(f'[INFO] Running FedPer with {self.model} model')
  ...
  def train(self):
    ...
    def batch_update(key, 
      ...
      grad_fn = jax.value_and_grad(shared_loss_fn, (0,1), has_aux = True)
      (loss, net_state), mean_grad = grad_fn(shared_params,
                                             personalized_params,
                                             net_state,
                                             key, batch)
      (shared_updates, per_updates), opt_state = opt.update(mean_grad, opt_state)

      shared_params = optax.apply_updates(shared_params,  shared_updates)
      personalized_params = optax.apply_updates(personalized_params, per_updates)

      return key, shared_params, personalized_params, opt_state, net_state
    ...
    for i in tqdm(range(self.num_rounds + 1),
      ...
      for t in chosen_idx:
        ...
        opt_state = opt.init((local_shared_params[t], local_per_params[t]))
        #net_state_t = local_net_state[t]

        for batch_idx, batch in enumerate(shared_batches):
          key, local_shared_params[t], \
              local_per_params[t], \
                opt_state, local_net_state[t] = batch_update(key,
                                                             local_shared_params[t],
                                                             local_per_params[t],
                                                             batch_idx,
                                                             opt_state,
      ...
      shared_params = jax_utils.model_average(round_local_updates, weights=round_weight_updates)
      net_state = jax_utils.model_average(round_net_state_updates, weights=round_weight_updates)

      if i % self.args['eval_every'] == 0:
        train_accu, test_accu = self.eval_fedper(shared_params, local_per_params, net_state, i)
  ...
  def eval_fedper(self, params, local_per_params, net_state, round_idx):
