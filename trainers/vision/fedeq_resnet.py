...
from utils.tree_utils import tree_l2_norm, tree_zeros_like
from utils.tree_utils import tree_random_normal_like, tree_inf_norm

from trainers.base import BaseTrainerGlobal


class FeDEQ_ResNet(BaseTrainerGlobal):
  def __init__(self, args, data):
    ...
    if self.dataset in ('femnist', 'cifar10', 'cifar100'):
      self.data_loss_fn = jax_utils.surrogate_loss_hk_resnet
      self.per_data_loss_fn = jax_utils.perdeq_sce_loss_hk_resnet
  ...
  def train(self):
    ...
    print(f'{num_params} shared params, size: {byte_size / 1e6:.2f}MB')
    ...
    print(f'{num_per_params} personalized params, size: {byte_size_per / 1e6:.2f}MB')
    ...
    if self.lam_admm == -1.0:
      ...
      lam_list = [tree_zeros_like(shared_params)] * self.num_clients
    else:
      print(f'[INFO] Lambda ADMM will be initalized as {self.lam_admm} for all clients')
      lam_list = [tree_scalar_mul(self.lam_admm, tree_zeros_like(shared_params))] * self.num_clients
    ...
    #dot = hk.experimental.to_dot(self.model.apply)(global_params, net_state, key, data_batch)
    #src = graphviz.Source(dot, format='png')
    #pic_path = src.render(filename="fedavg_resnet18", directory=self.args['outdir'])
    ...
    def shared_loss_fn(deq_params,
      ...
      train_term, net_state = self.data_loss_fn(deq_params,
                                                personalized_params,
                                                shared_params,
                                                net_state,
                                                rng, 
    ...
    def batch_update(key, 
      ...
      (loss, net_state), mean_grad = grad_fn(deq_params,
                                             personalized_params,
                                             shared_params,
                                             net_state,
                                             key, batch, lam, rho)
      ...
      deq_params = optax.apply_updates(deq_params, updates)

      return key, deq_params, opt_state, net_state
    ...
    for i in tqdm(range(self.num_rounds + 1),
      ...
      net_state = jax_utils.model_average(round_net_state_updates,
                                          weights=round_weight_updates)   
      ...
      if i % self.args['eval_every'] == 0:
        train_accu, test_accu = self.eval_fedeq(local_shared_params,
                                                local_per_params,
                                                local_net_state, 
                                                shared_params, 
                                                old_shared_params,
    ...
    if self.num_unseen != 0:
      ...
      #selected_clients = list(range(self.num_clients, self.num_total_clients))  
      #for t in chosen_idx:
  ...
  def eval_fedeq(self, local_shared_params, local_per_params, local_net_states, 
                 shared_params, old_shared_params, round_idx):
    ...
    for t in range(self.num_clients):
      ...
      res_client = tree_l2_norm(jax_utils.model_subtract(local_shared_params[t], shared_params), squared=True)
