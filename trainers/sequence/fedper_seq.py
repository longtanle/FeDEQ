...
class FedPer_SEQ(BaseTrainerGlobal):
  ...
  def train(self):
    ...
    def batch_update(key, 
      ...
      loss, mean_grad = jax.value_and_grad(shared_loss_fn, (0,1))(shared_params,
                                                                  personalized_params,
                                                                  batch,
                                                                  key)
      ...
      return key, shared_params, personalized_params, opt_state
  ...
  def eval_seq(self, params, local_per_params, round_idx):
    ...
    for t in range(self.num_clients):
      ...
      #train_loss = self.data_loss_fn(t_params, (self.x_train[t], self.y_train[t]), key)
      #train_losses.append(train_loss)
      # Test
      test_acc = self.pred_fn(t_params, key, self.x_test[t], self.y_test[t])
      num_correct_test.append(test_acc)
      #test_loss  = self.data_loss_fn(t_params, (self.x_test[t], self.y_test[t]), key)
